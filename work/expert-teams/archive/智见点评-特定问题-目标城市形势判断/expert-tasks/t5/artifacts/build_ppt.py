# -*- coding: utf-8 -*-
"""成都二手房形势研判（2025-09~2026-08）讨论稿 · 可编辑 PPTX 生成脚本
...
def hdr(slide, kicker, title, note=""):
    ...
    # 页脚
    add_text(slide, 0.55, 7.12, 9.5, 0.25, [("98wiki ｜ 智见 / 行业研究报告 · 成都二手房形势研判（讨论稿 · 2026-09-05）", 8, INK3, False)], sp_after=0)
    return 1.55
...
add_text(s, 0.9, 0.9, 11.5, 0.3, [("讨论稿 DISCUSSION DRAFT　·　数据截至 2026-08（8 月疑似未完月）", 11, GOLDL, False)], sp_after=0)
...
for v, l in kpis:
    ...
    x += 2.93
add_text(s, 0.9, 6.72, 11.6, 0.4, [("研判成员（匿名）：行业研究·D ｜ 宏观经济·F ｜ 宏观经济·L　·　口径：成交价=政研通贝壳交付（非挂牌价）；指数=贝壳官方房价指数（重复交易法）", 9.5, RGBColor(0x8F,0xA9,0xD6), False)], sp_after=0)
notes(s, "开场说明数据口径与讨论稿性质：本页为封面。核心信息：成都二手房进入底部区域但未确认；判定交给量价连续三个月交叉验证。数据时段 2025-09 至 2026-08，城市级二手房，成交价为贝壳交付口径而非挂牌价；8 月疑似未完月。")
...
add_text(s, 0.75, y+2.18, 11.9, 0.36, [([("三份研判收敛：", 12, BLUED, True), ("成都二手处于跌幅收窄的减速下行段；底部区域成立、右侧信号未现。", 12, INK2, False)], 12, INK2, False)], sp_after=0)
...
# ============ S9 三类决策 ============
s = prs.slides.add_slide(BLANK)
y = hdr(s, "问题③ · 三类决策落点", "刚需：可以开始看，不必抢跑", "每类都给出定价锚与边界条件（讨论稿 · 供参考）")
dec = [
...
g = [("单月不等于拐点", "3 月 12,849 套（+7.8%）被随后四个月连降证伪；8 月未完月同样不能单月定生死"),
     ...
     ("口径不能混用", "公开报道 4 月成都二手网签 24,498 套 ≈ 题供贝壳口径春季峰值的两倍；全网签 vs 平台活跃成交不可跨口径比绝对量，趋势只用同比/环比"),
...
# ============ S11 元信息与免责 ============
s = prs.slides.add_slide(BLANK)
y = hdr(s, "来源披露 · 补数路径 · 免责", "口径与后续观察项", "讨论稿 · 匿名标注（行业研究·D / 宏观经济·F / 宏观经济·L）")
...
add_text(s, 0.85, y+4.42, 11.7, 0.7, [("行业研究，不构成投资建议；测算/估算/研判推断非官方统计。数字为城市级口径，不代表具体小区或房源。", 11, WHITE, True),
                                     ("框架：用户视角五层（结论→记忆点→条件清单→补数→防误导→决策落点）；主基调由三份研判收敛，偏离观点降级为边界条件/风险提示。", 8.5, RGBColor(0xB9,0xCD,0xEF), False)], sp_after=3)
notes(s, "收尾页：统一披露口径与来源；六个补数观察项需要后续数据（8 月终值、去化分母定义、分总价段时序等）；免责声明：行业研究不构成投资建议，所有预测为研判推断。")

prs.save('chengdu-resale-2026-08.pptx')
