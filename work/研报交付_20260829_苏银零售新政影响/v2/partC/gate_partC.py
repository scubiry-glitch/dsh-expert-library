#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""99wiki Part C 分型验收器 v2（wiki-gated-research §Part C · logic-annex-v2）
改造自 v1 partC/gate_partC.py 审计框架（既定验收器，不重新试错）。
每次运行：weasyprint 重渲染 PDF → playwright/Chromium 重截图（宽屏元素级）→ fitz 重栅格化 A4
→ 重算 HTML/PDF 哈希 → Chromium DOM 检查 → 写 gate_partC.json。
纪律：DOM 优先，nested_p_count 等一律以 Chromium 加载后的 DOM 为准，禁止正则判嵌套；
Part C 不检查主文页数/封面/KPI/正文 10000 字门（那是 Part B 验收器的事）。
v2 增补：六类来源徽标（Wind/《意见》/驾驶舱·内部资料/政研通·贝壳/测算/推断）覆盖检查。"""
import hashlib, json, subprocess, sys, datetime, re
from pathlib import Path

BASE = Path("/root/zhijian/dsh-expert-library/work/研报交付_20260829_苏银零售新政影响/v2/partC")
HTML = BASE / "逻辑附件.html"
PDF = BASE / "逻辑附件.pdf"
SHOTS = BASE / "shots"
...
BADGE_CLASSES_REQUIRED = ["b-wind", "b-doc", "b-int", "b-bk", "b-est", "b-inf"]
...
def dom_and_wide_shots():
    ...
    with sync_playwright() as pw:
        ...
        d = page.evaluate("""() => {
          ...
          // —— 来源徽标覆盖（v2 增补）——
          const badgeCounts = {};
          for (const b of ['b-wind','b-doc','b-int','b-bk','b-est','b-inf','b-calc'])
            badgeCounts[b] = document.querySelectorAll('.src.' + b).length;
          // —— 详细卡逐项完整性 ——
          ...
          return {
            ...
            old_sig_hits: ["政研通研究院","智见行业研究 ｜ 行业研究报告"].filter(s => bodyText.includes(s)),
...
def main():
    ...
    badges_ok = all(d["badge_counts"].get(b, 0) >= 1 for b in BADGE_CLASSES_REQUIRED)
    ...
    checks = [
        ...
        {"item": "补数协议 = 1 个独立区块（4 项缺口 + 8 项观察指标，不计入 15 卡；v2：政研通已恢复不再列缺）",
        ...
        {"item": "来源徽标六类覆盖（Wind/《意见》/驾驶舱·内部资料/政研通·贝壳/测算/推断 均出现）",
         "pass": badges_ok, "detail": json.dumps(d["badge_counts"], ensure_ascii=False)},
    ...
    gate = {
        ...
        "content_source": "内容底稿_苏银零售新政影响_v2.md（唯一内容源；v1 结构口径结论作废）",
        "deliverable": {"html": str(HTML), "pdf": str(PDF), "gate": str(GATE), "shots": str(SHOTS),
                        "version_dir": "v2/partC（本轮 v2 交付目录）"},
        ...
        "count_proofs": {
            ...
            "protocol": {"blocks": d["protocol_blocks"], "items": d["proto_items"], "observation_indicators": d["obs_count"],
                         ...
                         "note": "v2 更新：政研通通道已恢复（旧缺口①作废）；底数仪表盘统一期间=2026Q2；4 项缺口（沿用底稿编号②③④⑤）=不良率明细/零售AUM/南京统计局销售面积/期间统一待补（驾驶舱 6-30 期末快照·同业 26Q2·网贷期末余额核验 2,790-2,850 亿推算）"},
            "source_badges": {"required": BADGE_CLASSES_REQUIRED, "counts": d["badge_counts"]},
        },
        "dom_checks": {
        ...
        "screenshots": {
            "wide": [{"path": str(p), "target": t, "checked": True,
                     ...
                     for p, t in zip(wide, ("首：附件头+导读（hero，含双口径与敏感边界）", "中：详细逻辑卡 08（卖旧买新资金错配与合规桥接）", "末：补数协议独立区块（v2 更新版 4 缺口+8 指标）"))],
