#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""99wiki Part B 分型验收器（wiki-gated-research §Part B）
每次运行：重新生成 PDF（weasyprint）→ 重新截图（playwright/Chromium 宽屏 + fitz A4）→ 重算 HTML/PDF 哈希 → DOM 检查 → 写 gate_partB.json
禁止正则判嵌套：nested_p_count 一律以 Chromium 加载后的 DOM 为准。"""
import hashlib, json, subprocess, sys, datetime, re, os
from pathlib import Path

BASE = Path("/root/zhijian/dsh-expert-library/work/研报交付_20260829_苏银零售新政影响/partB")
HTML = BASE / "主文.html"
PDF = BASE / "主文.pdf"
SHOTS = BASE / "shots"
...
def render_pdf():
    ...
    return {"time": datetime.datetime.now().isoformat(timespec="seconds"),
            "method": f"weasyprint {__import__('weasyprint').__version__} · A4 portrait · source=主文.html @media print"}
    ...
    with sync_playwright() as pw:
        ...
        d = page.evaluate("""() => {
          ...
          const perPage = pages.map((pg, i) => {
            ...
            return {
              page: i + 1,
              visuals: vis, tables: tbl, svg: svg, mech: mech,
...
def main():
    ...
    pdf_pages, a4_shots = raster_a4([0, 5, 11])
    ...
    for row in d["perPage"]:
        ...
        visuals_per_page.append({
            ...
            "visuals": row["visuals"], "tables": row["tables"], "svg": row["svg"], "mech": row["mech"],
            "cover_exempt": is_cover, "pass": passed,
        })

    body_ok = [r for r in d["perPage"] if r["page"] > 1]
    checks = [
        {"item": "页数计划（10-14 页范围）", "pass": 10 <= d["page_count"] <= 14,
         "detail": f"HTML .page = {d['page_count']}；PDF 页数 = {pdf_pages}；计划=12 页"},
        {"item": "nested_p_count == 0（DOM 标准）", "pass": d["nested_p_count"] == 0, "detail": str(d["nested_p_count"])},
        {"item": "无 JS 错误", "pass": not js_errors and not console_errs,
         "detail": f"pageerror={js_errors}; console.error={console_errs}"},
        {"item": "除封面每页 ≥1 主题视觉对象", "pass": all(r["pass"] for r in visuals_per_page),
         "detail": "; ".join(f"P{r['page']:02d}({r['role']}):{r['visuals']}{'✓豁免' if r['cover_exempt'] else ''}" for r in visuals_per_page)},
        ...
        {"item": "PDF 页数与页级骨架一致（12）", "pass": pdf_pages == d["page_count"], "detail": f"PDF={pdf_pages} HTML={d['page_count']}"},
    ...
    gate = {
        "type": "Part B 主文",
        "standard": "wiki-gated-research · 99wiki 分型验收（Part B 验收器）",
        "generated_at": datetime.datetime.now().isoformat(timespec="seconds"),
        "deliverable": {"html": str(HTML), "pdf": str(PDF), "gate": str(GATE), "shots": str(SHOTS)},
        "page_count_plan": "12 页（在 10-14 页范围内）：P01 封面 + P02-P11 正文 10 页（传导总图/底数仪表/战略/客群/产品/资产/风险/区域/行动/观察指标）+ P12 尾页来源披露",
    ...
    print(json.dumps({k: gate[k] for k in ("type", "result", "page_count_html", "page_count_pdf",
                                           "html_sha256", "pdf_sha256", "body_cjk_chars")}, ensure_ascii=False, indent=2))
    for c in checks:
        print(("PASS " if c["pass"] else "FAIL ") + c["item"] + "  —— " + str(c["detail"])[:160])
