/**
 * Expert Teams conversation card: a lightweight in-conversation summary shown
 ...
 * records for `expert_teams_create`. Those are first-party session events, so
...
export interface ExpertTeamsCardData {
...
declare module '@deepseek-ai/dsh-client-ui-conversation/client' {
  interface ChatNodeDataMap {
    ...
    'expert-teams': ExpertTeamsCardData
  }
}

/** Folded team record (the node's business state). */
export interface ExpertTeamsNodeState {
  readonly teamId: string
  readonly name: string
...
export function parseExpertTeamsCreateArgs(value: string): { teamId: string; name: string } | undefined {
  try {
...
export const agentTeamsCardDefinition: ConversationNodeDefinition<ExpertTeamsNodeState> = {
  kind: 'expert-teams',
  target: 'chat',
  match: (event) => {
    if (event.type === 'tool/call' && event.data.name === 'expert_teams_create') {
      return parseExpertTeamsCreateArgs(event.data.arguments) === undefined
        ? null
  ...
  start: (_context, match) => {
    if (match.event.type !== 'tool/call') {
      throw new Error('expert-teams card start requires expert_teams_create tool/call')
    }
    const parsed = parseExpertTeamsCreateArgs(match.event.data.arguments)
    if (parsed === undefined) throw new Error('expert-teams card start requires valid create arguments')
  ...
  buildViewNode: (context): ChatConversationViewNode | null => {
    ...
    const state = context.state as ExpertTeamsNodeState
