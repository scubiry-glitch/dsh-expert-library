/**
 * Expert Teams activity panel: the top-right floater monitoring every team.
 *
 ...
 * polls the host `/plugins/dsh-expert-library/state` route for
 ...
 * @module dsh-expert-library/client/activity
 */
...
const STATE_URL = '/plugins/dsh-expert-library/state'
...
function CollapsedBadge({ count, busy, onClick }: {
  ...
  return (
    <button type="button" className={css.badge} data-busy={busy} onClick={onClick} aria-label={`Expert Teams 活动，${count} 个团队`}>
...
function historicCardTeam(data: ExpertTeamsCardData, owner: string): ActivityTeam {
  return {
    workspace: '',
    teamId: data.teamId,
    name: data.teamName,
...
export function ActivityPanel({ sessionsList, openSession }: {
  ...
  return (
    <>
      ...
      {expanded && (
        <aside className={css.panel} data-expert-teams-activity>
