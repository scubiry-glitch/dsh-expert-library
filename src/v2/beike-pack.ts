 ...
 * pipeline 难而正确君/新药参谋），复用同一投影/模板/质量/方法构建器与发射器
 ...
 * - bk-002 制度君（制度派·经纪/居住服务）
 * - bk-018 统筹会长（存量统筹派·经纪/居住服务）
 * - bk-019 海外瞭望员（海外监管派·经纪管理）
 * - bk-031 挂牌哨探（挂牌量派·贝壳/NIFD 口径，internalOnly）
 * - bk-033 渠道灯塔（渠道崛起派·贝壳研究院院长）
 * - e08-08 难而正确君（贝壳创始人·平台经济）
 * - e04-05 新药参谋（居住服务·住房租赁/长租资管）
 * - s-07 组织一号位（组织设计·商业模式创新·数字化转型）
 * - s-13 斗嘴金主（平台商业模式·单位经济·网络效应）
 * - s-23 出行老舰长（双边平台·规模经济·运营执行）
 * - bk-016 结构老市长（前市长·制度与产业政策设计）
 * - bk-027 稳心会长（住建部/房协政策语境，引用须标注时点）
 * - bk-028 好房子工匠（住建部原总经济师·住房品质与治理）
...
const BEIKE_SCENARIOS: readonly ZhijianRouteScenario[] = [
  {
    ...
    constraints: '贝壳生态研判：渠道灯塔 bk-033（贝壳研究院院长）渠道/存量流通主答；难而正确君 e08-08（贝壳创始人）平台/服务品质/产业互联网；统筹会长 bk-018 存量统筹、制度君 bk-002 经纪制度、海外瞭望员 bk-019 监管对比作辅。挂牌哨探 bk-031 贝壳/NIFD 口径仅内部使用。',
  },
  {
    ...
    constraints: '长租/供应链合作（如贝壳×梦百合长租供应链）：新药参谋 e04-05 住房租赁/长租资管主答；渠道灯塔 bk-033 租赁市场/租购同权；难而正确君 e08-08 平台合作视角。素材：feishu 贝壳与梦百合长租供应链合作洽谈纪要。',
  },
]
...
function beikeDomainKnowledgeManifest(packVersion: string): DomainKnowledgeManifest {
  ...
  return {
    ...
    schemaVersion: SCHEMA_VERSION,
    domain: 'beike',
    boundary: '贝壳生态专家 Profile 投影（2026-08-23）：bk-002/018/019/031/033 居住服务与经纪 + e08-08 难而正确君（贝壳创始人）+ e04-05 新药参谋（长租资管）+ s-07 组织一号位/s-13 斗嘴金主/s-23 出行老舰长（商业模式与平台经济）+ bk-016 结构老市长/bk-027 稳心会长/bk-028 好房子工匠（政策治理，引用标注时点）。',
    ontology: {
      entities: [
...
function beikeProtocolMethodPack(packVersion: string): MethodPack {
  return {
    ...
    body: [
      '贝壳生态研判协议：',
      '1. 区分平台视角（难而正确君/渠道灯塔）与经纪制度视角（制度君/海外瞭望员/统筹会长），按问题落点选主答；',
      '2. 口径：贝壳成出口径（bk-031 挂牌哨探）与克而瑞/中指/统计局均有差异，须标注；',
      '3. 挂牌哨探 bk-031 为内测对比项，对外交付不引用；',
      '4. 商业与平台经济议题可选张勇 s-07（组织/商业模式）、斗嘴金主 s-13（单位经济/网络效应）、出行老舰长 s-23（双边平台/运营执行），分别校验组织生态、单位经济与平台执行；',
      '5. 政策治理议题可选黄奇帆 bk-016、稳心会长 bk-027、好房子工匠 bk-028；引用必须标注前任/原任身份和观点时点，不得表述为现任官员意见；',
