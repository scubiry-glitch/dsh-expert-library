...
- **AI console shell (rail + run stream + evidence aside):** for pages you **delegate on** rather than read or operate — AI 工作台, agent consoles, copilot UI. Three columns (`~276px | minmax(0,1fr) | ~344px`) mapping to the three tenses: queue and history on the left, the live run in the middle, artifacts and approvals on the right. The centrepiece is a **run stream**, not a chart, and the primary action is **停止**, not a filter. Full recipe — the nine run states, the in-stream approval card, the receipt line, the phone form — in **`ai-console.md`**. Example: `examples/relay-agent-console.html`.
