...
- `[HARD BAN]` **Re-drawn environment chrome** — hand-building the shell the user's own environment already provides: a fake browser bar (URL pill + three traffic-light dots), a fake phone frame (rounded rect + notch + speaker slit), a fake IDE (file tabs + activity bar + sidebar), a fake terminal window (title bar + dots wrapping a `<pre>`), a fake code-block window. → a real screenshot in a `<figure>` (at most a hairline border), or **no chrome at all** — let the content stand on its own.
  - **This is a different ban from fake screenshots above.** That one is about faking *the product's* interface; this one is about redrawing *the environment's*. A page can pass that ban and fail this one — a real screenshot glued inside a hand-drawn MacBook bezel is still re-drawn chrome.
  - **Boundary (not a violation):** the same test as above — if the chrome **is the product's own feature** (a browser-testing tool showing device frames, a code-sandbox product whose canvas is an editor), it's a component, not a prop. → single color, or weight/size emphasis. *(Exception: a deliberate duotone-soul move with the two locked accents.)*
...
- **Mid-build token improvisation** — a literal `#hex` / `oklch()` / `rgb()` / `hsl()`, or a `font-family: "Some Font"`, written *outside* the `:root` token block once the palette is locked. This is the **mechanism** behind accent drift above: nobody decides to break the color lock, they just get 400 lines into the CSS, need a slightly darker border, and type a hex. The lock is a decision; this is what enforces it. → every color and every font in the artifact references a named token (`var(--accent)`, `var(--font-display)`). Need a value that doesn't exist yet? **Lift it into the token block as a new named variable, then reference it** — never inline it at the use site. Mechanical check: grep for color literals outside `:root`; a handful in a hand-tuned gradient or an SVG fill is fine, a scatter of them across component rules means the palette stopped being a system somewhere in the middle.
...
**Imagery**
- **Zero imagery where the page has an image slot.** Judge this by the **skeleton you actually built, not by the industry** — a hero that wants a photograph behind it, a gallery/lookbook rail, a PDP shot, an H5 cover or ambient scene, a "who we are" band, a dashboard's empty state or avatar row. Food/hotel/fashion/travel/product briefs are the *obvious* cases, not the boundary: a slot filled with a gradient because nobody raised the question is the same bug in any register. That's a bug, not minimalism. → real/generated photography; even restrained editorial needs 2–3 real images. **And this check firing at delivery means it fired late** — the slots were supposed to be named at the Design Read (`SKILL.md` §0.B's `Images:` line), where the user could still answer cheaply. See `asset-sourcing.md` for the generate-vs-stock-vs-placeholder decision, and note its gate: you **ask** and wait, you never generate or download on your own read of the brief.
