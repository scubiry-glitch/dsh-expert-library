...
test('pipeline-domains source parses into e01/e08 namespaced metas (E13 moved to bank)', async () => {
  ...
  assert.equal(parsed.experts.length, 106)
  ...
  assert.ok(ids.includes('e07-07'), 'E07 吴恩达 已收')
  assert.ok(ids.includes('e02-09'), 'E02 计葵生 已收')
  ...
  assert.equal(general.experts.length, 41)
  const generalIds = general.experts.map(e => e.id)
  assert.ok(generalIds.includes('s-32') && generalIds.includes('s-40') && generalIds.includes('xhs-01'))
})
