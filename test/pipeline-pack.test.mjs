...
test('pipeline-domains pack validates clean with 22 experts', () => {
  ...
  assert.equal(pack.experts.length, 22)
...
test('routeRequest routes 银行战略与经营 to e13 candidates', () => {
  const result = routeRequest('银行战略与经营（银行战略、量化目标、数智化转型）')
  assert.ok(result.candidates.some(c => c.id === 'e13-02'))
})

test('zhijian-realestate pack slice excludes pipeline scenarios', async () => {
