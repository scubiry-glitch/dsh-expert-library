...
test('framework A: roster = selected ids, reviewer fan-out, fusion + render tasks unassigned', () => {
  ...
  // role.fusion is optional (min 0) → fusion + render stay in the shared pool.
  ...
  // + one unassigned render (shared pool).
  ...
  assert.deepEqual(expanded.tasks.map(task => task.assigneeExpertId), ['bk-024', 'bk-025', undefined, undefined])
  assert.deepEqual(expanded.tasks[2]?.dependsOn, ['t1', 't2'])
  assert.deepEqual(expanded.tasks[3]?.dependsOn, ['t3'])
...
test('review copy reproduces the V1 strings after interpolation (subject, steps, constraints, fusion)', () => {
  ...
  // the literal `5. ` fusion rules are folded in. Subject drops the 渲染 suffix
  // (render is now the separate t3 node).
  ...
  assert.equal(fusion?.subject, '融合合成（讨论稿/正式稿）')
  // Render node: three skills + completion checkpoint bound.
  const render = expanded.tasks[2]
  assert.equal(render?.subject, '渲染与生成（HTML5 → PDF/PPT → 视频）')
...
test('1 and 5 selected experts expand to 1..1 and 1..5 reviews', () => {
  ...
  if (one.ok) {
    ...
    assert.equal(expanded.tasks.length, 3)
  }
  ...
  if (five.ok) {
    ...
    assert.equal(expanded.tasks.length, 7)
